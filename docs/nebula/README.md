...
| [Project contract](PROJECT_CONTRACT.md)     | Domain engineering constraints under root AGENTS.md.                                    |
| [Architecture](ARCHITECTURE.md)             | Inherited runtime, target coordination architecture, and future concepts.               |
| [Swarm model](SWARM_MODEL.md)               | Shared runtime principles and supervised multi-agent policy.                            |
| [Runtime map](RUNTIME_MAP.md)               | Evidence-based map of the inherited T3 Code repository.                                 |
| [Extension contract](EXTENSION_CONTRACT.md) | Canonical attachment points, implemented boundaries, divergence budget, and ADR review. |
...
Nebula product docs describe what the product is and where it is going. Architecture docs distinguish inherited behavior from target behavior. The runtime map is an evidence snapshot; verify current attachment points before relying on it. The extension contract determines where Nebula may attach without creating competing systems.
...
For task authority and repository-wide safety, follow [AGENTS.md](../../AGENTS.md). The following order resolves descriptions of architecture and capability status; code behavior does not grant permission to bypass safeguards:
