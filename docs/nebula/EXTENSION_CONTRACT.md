...
| Task object                | No explicit Nebula Task; thread is the nearest durable work unit | `packages/contracts/src/orchestration.ts`; `apps/server/src/orchestration/`                                                                                                                                           | NEW NEBULA MODULE | Acceptable          | Medium | First implementation boundary. Task is not a renamed thread.                 |
| Writable task workspace    | Project workspace root plus optional thread worktree path        | `packages/contracts/src/orchestration.ts`; `apps/server/src/checkpointing/Utils.ts`                                                                                                                                   | EXTEND            | Acceptable          | High   | One writable Task owns one effective worktree lifecycle.                     |
...
| Path ownership             | Git changed paths exist; no task ownership policy                | Existing VCS diff plus future Task contracts                                                                                                                                                                          | NEW NEBULA MODULE | Acceptable          | High   | Mechanically validate owns/denies before approval/integration.               |
...
| Task status                | Thread lifecycle exists; no task lifecycle                       | `packages/contracts/src/orchestration.ts`; orchestration projections                                                                                                                                                  | EXTEND            | Acceptable          | Medium | Define small explicit states and reversible transitions.                     |
...
| Mission DAG                | No mission/task dependency domain                                | Existing orchestration engine is the runtime anchor                                                                                                                                                                   | DEFER             | Intentional later   | High   | v0.2 only; do not make a swarm-only runtime.                                 |
...
## Recommended first Nebula module

The smallest correct v0.1 implementation boundary is an **explicit Nebula Task tied to one existing thread/provider session and one existing workspace**, implemented as a narrow extension of the current orchestration domain.

Recommended initial placement, subject to implementation-time inspection:

- add Task schemas, commands, events, and read-model fields beside the existing contracts in `packages/contracts/src/orchestration.ts` or a nearby contract module imported by it;
- add Task decisions/projections to `apps/server/src/orchestration`, using the same engine, SQLite transaction, migrations, and subscriptions;
- place only task-specific composition/policy in `apps/server/src/nebula/` if the first service has enough cohesive behavior to justify a directory;
- bind the Task to an existing project, thread, provider instance/session, and effective workspace path; and
- expose it through existing RPC and client-runtime state.

Do **not** create `packages/nebula-*` for the first slice. The state transitions need server orchestration and existing infrastructure; a new package would add boundaries without current reuse. Do not begin with automatic worktree management, ownership enforcement, missions, or routing. First prove Task identity, provider/thread binding, status, event history, and visibility. The next slice can make a writable Task request an inherited worktree and checkpoint baseline.
