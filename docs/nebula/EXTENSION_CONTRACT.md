...
| Mission DAG                | Existing Task, event, projection, shell, and Command Deck domains                                           | Pure shared graph engine plus normalized Mission projections and existing Task commands                                                                                                                               | EXTEND           | Implemented         | High                                                                            | Human-authored DAG and manual waves; no scheduler or swarm-only runtime.                  |
...
No `packages/nebula-*` package or parallel runtime was created. Ownership enforcement, Integration, and Missions are implemented over stable Task/Result identity, hidden checkpoint refs, the inherited Git executor, process runner, typed commands, and event projection. Command Deck, Quality Gates, independent Reviewer, Mission membership, dependency edges, and manual wave starts use the same shell and persistence contracts. Agent ownership requests, shared-resource locking, automatic planning/scheduling/routing/remediation, automatic integration queues, Architect, and Swarm Mode remain not implemented.
