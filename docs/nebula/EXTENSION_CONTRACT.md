...
This contract classifies how Nebula attaches to the inherited runtime. `REUSE` means consume a primitive without changing its responsibility. `EXTEND` means add compatible contracts/state/behavior at its established boundary. `COMPOSE` means coordinate several existing primitives in a Nebula application service. `NEW NEBULA MODULE` means new product-domain behavior inside the existing server/client architecture. `DEFER` means explicitly unimplemented at the verified baseline, not a permanent prohibition. `DO NOT DUPLICATE` identifies infrastructure Nebula must not recreate.

## Capability map

This is attachment/status guidance, not execution authority. Follow [AGENTS.md](../../AGENTS.md) and verify affected implementation; historical release labels do not override the current task.
...
| Shared-resource locking      | Task resource requirements and durable Mission resource leases                                                   | Existing orchestration engine/persistence; shared Mission scheduler                                                                                                                                                   | IMPLEMENTED      | Intentional         | High   | Deterministic leases gate writable Task starts and reconcile from canonical Task state.                                                       |
...
| Independent Reviewer         | Provider-neutral TextGeneration plus bounded immutable review package and durable rounds                         | `TaskReviewReactor`; `taskIndependentReview.ts`; migration 046; Command Deck                                                                                                                                          | IMPLEMENTED      | Intentional         | High   | Read-only evidence analysis; cross-provider preferred, same-provider disclosed.                                                               |
...
| Mission DAG                  | Existing Task, event, projection, shell, and Command Deck domains                                                | Pure shared graph/replan engine plus normalized Mission projections and existing Task commands                                                                                                                        | EXTEND           | Implemented         | High   | Active changes require a validated, human-approved, atomic Plan-version transition.                                                           |
| Supervised Mission runner    | MissionRun commands/events/projection plus existing Task start and review commands                               | Shared deterministic scheduler; `MissionRunReactor`; Command Deck and Terminal Center                                                                                                                                 | IMPLEMENTED      | Intentional         | High   | Executes approved Plans; replans require human approval and separate apply. Bounded provider substitution follows authorized recovery policy. |
| Provider routing             | Existing provider registries and frozen Mission Run recovery/routing policy                                      | `apps/server/src/orchestration/Layers/MissionRunReactor.ts`; `packages/shared/src/recoveryRouting.ts`                                                                                                                 | IMPLEMENTED      | Intentional         | High   | Bounded selection and replacement use real readiness/capabilities under authorized Run policy; Task/worktree identity remains stable.         |
...
| Terminal Center/Agent Canvas | Existing Project routes, provider registry, Thread/Task commands, Mission DAG, ChatView, and UI preference store | `apps/web/src/routes/projects.$projectKey_.terminal-center.tsx`; `apps/web/src/components/terminalCenter/`; `apps/web/src/uiStateStore.ts`                                                                            | IMPLEMENTED      | Intentional UI      | Medium | UI-only spatial composition; no second provider, terminal, Task, Git, event, or persistence runtime.                                          |
...
No `packages/nebula-*` package or parallel runtime was created. Ownership enforcement, Integration, Missions, shared resources, Architect planning, supervised Mission execution, bounded recovery/routing, human-gated Plan versioning, and Supervised Swarm are implemented over stable Task/Result identity, hidden checkpoint refs, the inherited Git executor, process runner, typed commands, and event projection. Command Deck, Quality Gates, independent Reviewer, Mission membership, dependency edges, MissionRun scheduling, automatic wave advancement, provider substitution, bounded Replan application, and Automatic Integration use the same shell and persistence contracts. Provider-generated ownership grants, unapproved plan rewriting, AI conflict resolution, automatic publication, and automatic `main` merge remain unimplemented.
...
- shared contracts with Task and coordination schemas;
...
This reference maps visual attachment points and compatibility risks. It neither authorizes rebranding nor prohibits a separately requested change; current task scope controls authority.
