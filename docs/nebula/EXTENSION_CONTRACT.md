...
| Gemini CLI provider        | ProviderDriver plus shared ACP session runtime                                                              | `apps/server/src/provider/Drivers/GeminiDriver.ts`; `apps/server/src/provider/Layers/GeminiAdapter.ts`; `apps/server/src/provider/acp/AcpSessionRuntime.ts`                                                           | IN PROGRESS       | Low                 | Automated ACP contracts pass; live Google auth is blocked for this account. |
