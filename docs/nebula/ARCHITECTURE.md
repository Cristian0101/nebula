...
Operational recovery follows **UI displays, runtime decides, persistence recovers**. The Mission Command Center derives discrete progress, attention links, Task attempt history, review coverage, resource queues, Integration state, and searchable timeline rows from canonical projections. It does not persist renderer-only Task states or a second Mission event log. Automatic recovery is limited to one transient transport retry. Provider authentication, provider execution, quality, review, ownership, resource, workspace, Integration, and policy failures require attention; provider replacement and remediation are deliberate user actions.
