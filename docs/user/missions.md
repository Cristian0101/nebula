...
Task handoffs keep every historical risk. The final report separately shows **Remaining risks** and **Resolved during Mission**. A risk leaves the remaining set when a captured Integration remediation commit contains an exact `Nebula-Resolved-Risk: <historical risk>` trailer. Nebula can also reconcile the narrow built-in warnings for a named missing artifact when that exact path or artifact is present in the integrated files and every current review and required final gate passes. A missing required-test-evidence warning resolves only when the same current Task snapshot has a required canonical quality pass and an approving cross-provider review; the warning remains historical and is omitted from current downstream context. The exact missing Builder-evidence note is resolved only after canonical reviews and gates replace it. Passing gates, a later approval, or an unrelated Integration change does not erase any other warning by itself. Older persisted reports keep their original legacy known-risk presentation and are not rewritten.
