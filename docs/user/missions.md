...
When the canonical completion criteria pass, Nebula persists both the Mission and Run as **Completed** and stores the factual final report with the Run. Mission History reconstructs every Plan version, Replan trigger and scope, graph, Tasks, attempts, reviews, Integration Batch, final gates, exact Integration SHA, and final report from canonical events after reload, runtime restart, or a new frontend session. The report counts applied Replans separately from provider replacements, retries, and review remediation, and identifies preserved, modified, added, and superseded Tasks. It also counts deliberate operator actions such as provider replacement, sent review remediation, resolved ownership or coordination requests, Integration intervention, and human Plan edits; automatic scheduling and bounded retry are not human interventions.

Task handoffs keep every historical risk. The final report separately shows **Remaining risks** and **Resolved during Mission**. A risk leaves the remaining set when a captured Integration remediation commit contains an exact `Nebula-Resolved-Risk: <historical risk>` trailer. Nebula can also reconcile the narrow built-in warnings for a named missing or untracked artifact when every named path is present in the integrated files and every current review and required final gate passes. A historical full-validation failure resolves only when the current required reviews and final gates provide replacement evidence. A missing required-test-evidence warning resolves only when the same current Task snapshot has a required canonical quality pass and an approving cross-provider review; the warning remains historical and is omitted from current downstream context. The exact missing Builder-evidence note is resolved only after canonical reviews and gates replace it. Passing gates, a later approval, or an unrelated Integration change does not erase any other warning by itself. On startup, Nebula may idempotently refresh a completed report from the same canonical Mission, Task, review, and Integration history so older derived counts and risk buckets match that retained evidence; it cannot reopen the completed Run or alter its completion timestamp.
