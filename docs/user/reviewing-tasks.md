...
For requested changes in a manual Task, choose **Send Review Findings to Builder** to place a concise, traceable message in the existing Builder Thread. After the Builder changes the Task, prepare a fresh snapshot, rerun required gates, and request another round. Earlier rounds remain in history. An explicitly authorized Supervised Swarm Run may perform the same cycle automatically within its frozen remediation budget; exhausted rounds stop for attention.
