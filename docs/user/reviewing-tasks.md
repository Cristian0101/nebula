...
A required-review Task can complete only when ownership is valid, the snapshot and ready handoff are current, all required gates for that snapshot passed, and a current round is **Approve** or **Approve with notes**. That completed result is then eligible for a manual Integration Batch. Shared-resource locks, Mission/DAG planning, scheduling, automatic routing/remediation, automatic integration queues, and Swarm Mode are not implemented.
