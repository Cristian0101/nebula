...
A required-review Task can complete only when ownership is valid, the snapshot and ready handoff are current, all required gates for that snapshot passed, and a current round is **Approve** or **Approve with notes**. That completed result satisfies downstream Mission dependencies and is eligible for a manual Integration Batch. Mission waves do not weaken Task review policy or start remediation automatically. Shared-resource locks, automatic planning/scheduling/routing/remediation, automatic integration queues, Architect, and Swarm Mode are not implemented.
