  ...
  if (!url)
    return (
      <div className="grid h-full place-items-center p-6 text-center text-xs text-muted-foreground">
        Select a running Dev Server to attach this Preview.
      </div>
    );
  return (
...
export function ProjectTerminalWorkspace({
  ...
  const [devSuggestionOpen, setDevSuggestionOpen] = useState(false);
  ...
  const [freeformDrag, setFreeformDrag] = useState<{
    paneId: string;
    startX: number;
    startY: number;
    originX: number;
  ...
  const addPane = useCallback(
    (
      ...
      const placement = firstAvailableGridPlacement(currentWorkspace.panes, addAt ?? undefined);
      if (!placement) {
        reportError(
  ...
  const startProfile = useCallback(
    async (
      ...
      if (restart) {
        await closeTerminal({
          environmentId: project.environmentId,
          input: { threadId: hostThreadId, terminalId, deleteHistory: false },
        });
      }
      ...
      if (commandFailure(opened)) {
        reportError("Could not start Dev Server", "Nebula could not open its approved PTY.");
        return;
      }
  ...
  const stageDevPane = stagePane?.attachedPaneId
    ...
    : stageProfile
      ...
      : null;
  const stageUrl = stagePane?.previewUrl || stageProfile?.previewUrl || "";
  ...
  const renderPaneContent = (pane: TerminalWorkspacePane, active: boolean) => {
    ...
    const serverVerified =
      profile?.preferredPort === null || profile?.preferredPort === undefined
        ? processStatus === "Running"
        : discoveredServers.some((server) => server.port === profile.preferredPort);
    const status = processStatus === "Running" && !serverVerified ? "Starting" : processStatus;
    return (
      <div className="flex h-full min-h-0 flex-col">
                ...
                <Button
                  ...
                  onClick={() => {
                    const existingPreview = activeWorkspace.panes.find(
                      (candidate) =>
                        candidate.type === "preview" && candidate.attachedPaneId === pane.id,
                    );
                    if (existingPreview) {
      ...
      <SidebarInset className="flex h-dvh min-h-0 flex-col overflow-hidden bg-background text-foreground">
        ...
        <div className="grid min-h-0 flex-1 grid-rows-[minmax(0,3fr)_minmax(220px,1fr)] gap-2 p-2 lg:grid-cols-[minmax(0,3fr)_minmax(300px,1fr)] lg:grid-rows-1">
          <div className="min-h-0 overflow-hidden rounded-xl border border-border bg-card shadow-sm">
            <PreviewSurface url={stageUrl} title="Preview Stage" refreshKey={previewRefreshKey} />
          </div>
  ...
  return (
    <SidebarInset className="flex h-dvh min-h-0 flex-col overflow-hidden bg-background text-foreground">
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-1 border-b border-border px-3 py-2">
        ...
        <div className="ml-auto flex items-center rounded-md border border-border bg-muted/20 p-0.5">
          <Button
            size="xs"
      ...
      <div
        ...
        onPointerMove={(event) => {
          ...
          updateActiveWorkspace((workspace) => ({
            ...
            panes: workspace.panes.map((pane) =>
              pane.id === freeformDrag.paneId
                ? {
                    ...pane,
                    freeform: {
                      ...pane.freeform,
                      x: freeformDrag.originX + event.clientX - freeformDrag.startX,
                      y: freeformDrag.originY + event.clientY - freeformDrag.startY,
                    },
                  }
                : pane,
            ),
        ...
        onPointerUp={() => setFreeformDrag(null)}
      >
        <div
          className={
            focusedPane
              ...
              : activeWorkspace.layout === "grid"
                ? "grid h-full min-h-[640px] grid-cols-4 grid-rows-4 gap-2 p-2"
        ...
        >
          {!focusedPane && activeWorkspace.layout === "grid"
            ? Array.from({ length: 16 }, (_, index) => {
                ...
                return (
                  <button
                    ...
                    className="group z-0 grid min-h-28 place-items-center rounded-xl border border-dashed border-border/70 bg-card/25 text-xs text-muted-foreground hover:border-primary/60 hover:bg-primary/5 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          ...
          {(focusedPane ? [focusedPane] : visiblePanes).map((pane) => {
            ...
            const paneStyle = focusedPane
              ? undefined
              : activeWorkspace.layout === "grid"
                ? {
                    gridColumn: `${pane.grid.column} / span ${pane.grid.columnSpan}`,
                    gridRow: `${pane.grid.row} / span ${pane.grid.rowSpan}`,
                  }
                : activeWorkspace.layout === "freeform"
                  ? {
                      position: "absolute" as const,
                      left: pane.freeform.x,
                      top: pane.freeform.y,
            ...
            return (
              <article
                ...
                className={`z-10 flex min-h-0 min-w-0 flex-col overflow-hidden rounded-xl border bg-card shadow-sm ${selected ? "border-primary ring-1 ring-primary/35" : "border-border"}`}
                style={paneStyle}
                onFocus={() =>
              ...
              >
                ...
                >
                  {renderPaneContent(pane, selected || Boolean(focusedPane))}
                </div>
              </article>
            );
          })}
      ...
      <Dialog open={addPaneOpen} onOpenChange={setAddPaneOpen}>
        <DialogPopup>
          <DialogPanel className="max-w-3xl">
            <DialogHeader>
              <DialogTitle>New Pane</DialogTitle>
              <DialogDescription>
                Add a live surface to {activeWorkspace.name}. Terminal Workspaces arrange UI; they
                do not create Git worktrees.
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4 md:grid-cols-2">
              <section>
                <p className="mb-2 text-xs font-medium text-muted-foreground">Fast options</p>
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      devServerProfiles.length > 0
                        ...
                        : (setAddPaneOpen(false), setDevSuggestionOpen(true))
                    }
                  >
                    <AppWindowIcon /> Dev Server
                  </Button>
                  <Button
                    variant="outline"
                    className="justify-start"
                    disabled={devServerProfiles.length === 0}
                    onClick={() => {
                      const profile = devServerProfiles[0];
                      if (profile)
                        addPane({
                          type: "preview",
                          title: `${profile.name} Preview`,
                          ...
                          attachedPaneId:
                            activeWorkspace.panes.find(
                              (pane) =>
                                pane.type === "dev_server" &&
                                pane.devServerProfileId === profile.id,
                            )?.id ?? null,
                        });
                    }}
                  >
                  ...
                  <Button
                    variant="outline"
                    className="justify-start"
                    onClick={() => addPane({ type: "git", title: "Git Status" })}
                  >
                    <GitBranchIcon /> Git Status
                  </Button>
                  <Button
                    variant="outline"
                    className="justify-start"
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setAddPaneOpen(false)}>
                Cancel
              </Button>
            </DialogFooter>
          </DialogPanel>
      ...
      <Dialog open={devSuggestionOpen} onOpenChange={setDevSuggestionOpen}>
        <DialogPopup>
          <DialogPanel>
            <DialogHeader>
              <DialogTitle>Suggested Dev Server</DialogTitle>
              <DialogDescription>
                Nebula inspected Project metadata. No discovered command runs until you approve it.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-2 py-4">
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 font-mono text-xs text-primary">{suggestion.command}</p>
                      <p className="mt-2 text-[11px] text-muted-foreground">
                        Working directory: {suggestion.workingDirectory} · Suggested from{" "}
                        {suggestion.source}
                      </p>
                    </div>
                    <Button size="xs" onClick={() => void approveSuggestion(suggestion)}>
                      Approve & Start
                    </Button>
                  </div>
                </div>
              ...
              {suggestions.length === 0 ? (
                <div className="rounded-lg border border-border p-4 text-sm text-muted-foreground">
                  No supported dev command was found. Add a profile in Project Settings or add a
                  package.json dev script.
                </div>
              ) : null}
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setDevSuggestionOpen(false)}>
                Cancel
              </Button>
            </DialogFooter>
          </DialogPanel>
